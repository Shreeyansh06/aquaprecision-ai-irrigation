...
async function startServer() {
  ...
  // Must be before express.static and the SPA catch-all so they are never intercepted.
  ...
  app.post("/step", express.json(), (req, res) => {
    ...
    if (action === "WATER" && cropId !== undefined && amount !== undefined) {
      ...
      if (crop && envState.reservoirLevel >= amount) {
        ...
        crop.moisture = Math.min(100, crop.moisture + amount);
        envState.reservoirLevel -= amount;
        reward = (crop.moisture - before) / 100;
        if (crop.moisture >= 40 && crop.moisture <= 80) reward += 0.5;
      }
    } else {
      const h = hours || 1;
      envState.hour += h;
      ...
      envState.crops?.forEach((c: any) => {
        c.moisture = Math.max(0, c.moisture - (envState.weatherForecast === "HEATWAVE" ? 4 : 2));
        c.health = c.moisture < 20 ? Math.max(0, c.health - 5) : Math.min(100, c.health + 1);
      });
      reward = (envState.crops?.filter((c: any) => c.moisture >= 40 && c.moisture <= 80).length || 0) * 0.1;
    }
    ...
    res.json({ state: envState, reward: parseFloat(reward.toFixed(3)), done: envState.done });
  });
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // ✅ IMPORTANT: Only serve index.html for non-API, non-OpenEnv routes
